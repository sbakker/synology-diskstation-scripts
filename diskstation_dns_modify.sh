...
# These settings should be edited to match the settings of your existing
# DNS server configurtion on your synology server
#   (Main Menu -> DNS Server -> Zones)
...
overridesettings(){
  # $1 is both script global variable name and the parameter name in
  # settings file.
  settingsfile=$(dirname $0)/settings

  if [ -r $settingsfile ]; then
    if ignoredresult=$(cat $settingsfile | grep $1=); then
      value=$(grep $1= $settingsfile | tail -1 | cut -f2 -d"=")
      eval "$1=$value"
...
# User specific settings are loaded from settings file, if present.
# This makes upgrading this script easier.
overridesettings YourNetworkName
overridesettings ForwardMasterFile
overridesettings ReverseMasterFile

# TODO: automagically determine filenames for forward and reverse zones,
# so that this file does not need to be edited to work in a default config.
...
[ -f $tmpPrefix.$ForwardMasterFile ] \
    && date_echo "INFO: Forward master already backed up for today." \
    || cp -a $ZonePath/$ForwardMasterFile $tmpPrefix.$ForwardMasterFile
[ -f $tmpPrefix.$ReverseMasterFile ] \
    && date_echo "INFO: Reverse master already backed up for today." \
    || cp -a $ZonePath/$ReverseMasterFile $tmpPrefix.$ReverseMasterFile

# Declare reusable functions.  Logic is pretty much the same for forward
# and reverse files.
printPartialDNSFile () {
...
printDhcpAsRecords () {
    ...
    awk -v YourNetworkName=$YourNetworkName \
        -v RecordType=$1  -v StaticRecords=$2 \
        -v adapters=$NetworkInterfaces '
        BEGIN {
...
incrementSerial () {
# serial number must be incremented in SOA record when DNS changes are
# made so that slaves will recognize a change

  # isolate DNS serial from first line following SOA
  ser=$(sed -e '1,/.*SOA/d' $1 | sed -e '2,$d' -e 's/;.*//' )

  # preserve any comments, if any exist
  comments=$(sed -e '1,/.*SOA/d' $1 \
    | sed -e '2,$d' | sed -n '/;/p' | sed -e 's/.*;//' )
...
# The only exception are "ns.domain" records.  We keep those.
# Assumptions:
#   PTR and A records should be removed unless they contain
#   "ns.<YourNetworkName>."
...
incrementSerial $BackupPath/$ForwardMasterFile.new \
    > $BackupPath/$ForwardMasterFile.bumped
...
incrementSerial $BackupPath/$ReverseMasterFile.new \
    > $BackupPath/$ReverseMasterFile.bumped

##########################################################################
# Ensure the owner/group and modes are set at default
# then overwrite the original files
date_echo "Overwriting with updated files:" \
    "$ForwardMasterFile $ReverseMasterFile"
if ! chown nobody:nobody \
    $BackupPath/$ForwardMasterFile.bumped \
    $BackupPath/$ReverseMasterFile.bumped
then
...
chmod 644 $BackupPath/$ForwardMasterFile.bumped \
    $BackupPath/$ReverseMasterFile.bumped
